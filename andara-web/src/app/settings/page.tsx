...
import { getGuideSettings, saveGuideSettings, addActivityLog } from "@/lib/services/crm"

export default function SettingsPage() {
  // Guide settings state (Immutable account info from session cookie)
  const [name, setName] = useState("Guía Demo")
  const [email, setEmail] = useState("guia@andara.pe")

  // Mutable guide profile details
  ...
  // Load session credentials and settings on mount
  useEffect(() => {
    let sessionUser = { name: "Guía Demo", email: "guia@andara.pe" }
    ...
    // Lock account settings to logged-in user
    setName(sessionUser.name)
    setEmail(sessionUser.email)

    const settings = getGuideSettings()
    setPhone(settings.phone || "+51999999999")
    setAgencyName(settings.agencyName || "Andara Tours")
    ...
    setWhatsappLink(settings.whatsappLink || "wa.me/51999999999")
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    const updated = {
      name, // Save immutable names so that it syncs correctly with the Sidebar badge
      email,
      phone,
      agencyName,
      location,
    ...
    saveGuideSettings(updated)
    addActivityLog(`Configuración de perfil de ${name} actualizada.`)
    
    // Dispatch event to refresh Sidebar profile
        ...
        .then(data => {
          if (data.success) {
            ...
            if (data.pages && data.pages.length > 0) {
              ...
              setWabaId(names)
              addActivityLog(`Se conectaron las páginas de Facebook: ${names}`)
            } else {
              setWabaId("Ninguna página encontrada")
              addActivityLog("No se encontraron páginas de Facebook para conectar.")
            }
          } else {
    ...
    <div className="max-w-2xl mx-auto space-y-8 pb-12">
      ...
      >
        <form onSubmit={handleSaveSettings} className="space-y-6">
          ...
          {/* Account info section (Disabled for Immutable Login Account verification) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-muted-foreground">Nombre de Usuario (Sesión Iniciada)</Label>
              <Input 
                id="name" 
                value={name} 
                disabled
                className="bg-muted/50 border-border/30 text-muted-foreground cursor-not-allowed"
                title="Este campo está bloqueado porque representa tu cuenta de registro oficial."
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="email" className="text-muted-foreground">Correo de Usuario (Sesión Iniciada)</Label>
              <Input 
                id="email" 
                type="email"
                value={email} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="agencyName">Nombre de tu Agencia (Opcional)</Label>
              <Input 
                id="agencyName" 
                value={agencyName} 
                onChange={e => setAgencyName(e.target.value)} 
                placeholder="Ej. Andara Tours" 
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="license">Licencia de Guía Oficial (DIRTUR ID)</Label>
              <Input 
                id="license" 
                value={license} 
                onChange={e => setLicense(e.target.value)} 
                placeholder="Ej. DIRTUR-ICA-00123" 
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="languages">Idiomas que Dominas</Label>
              <Input 
                id="languages" 
                value={languages} 
            ...
            <div className="space-y-2">
              <Label htmlFor="whatsapp_link">Enlace Directo de WhatsApp</Label>
              <Input 
                id="whatsapp_link" 
                value={whatsappLink} 
              ...
              />
              <p className="text-xs text-muted-foreground">Usado para abrir conversaciones directas con clientes.</p>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="bio">Presentación / Biografía Profesional</Label>
            <Textarea 
              id="bio" 
              value={bio} 
              onChange={e => setBio(e.target.value)} 
              placeholder="Cuéntales a tus clientes sobre tu experiencia y especialidades de tours..." 
        ...
        <div className="mt-8 pt-8 border-t border-border/40 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-foreground">Conectar Páginas de Facebook (Meta Developer)</h3>
          </div>
          
          <p className="text-sm text-muted-foreground leading-relaxed">
            Vincula tus páginas comerciales oficiales de Facebook de forma autónoma para recibir todos tus chats de Messenger e Instagram en tiempo real en Andara CRM.
          </p>
          ...
          <Button 
            ...
            className={`w-full text-white rounded-xl py-6 font-semibold shadow-lg transition-all cursor-pointer ${
              fbReady ? 'bg-[#1877f2] hover:bg-[#166fe5] shadow-blue-500/10' : 'bg-gray-600 cursor-not-allowed'
            }`}
          >
